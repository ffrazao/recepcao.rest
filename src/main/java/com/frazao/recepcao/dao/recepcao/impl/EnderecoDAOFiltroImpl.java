...
import com.frazao.recepcao.dao.recepcao.EnderecoDAOFiltro;
...
public class EnderecoDAOFiltroImpl implements EnderecoDAOFiltro {
	...
	@Override
	public Collection<Endereco> filtrar(final EnderecoFiltroDTO f) {

		final StringBuilder sql = new StringBuilder();
		sql.append("SELECT em.*").append("\n");
		sql.append("FROM   ").append(this.databaseSchema).append(".endereco as em").append("\n");
		...
		final Query query = this.entityManager.createNativeQuery(sql.toString(), Endereco.class);
		// if (StringUtils.isNotBlank(f.getCpfCnpj())) {
